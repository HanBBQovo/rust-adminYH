...
const backendRouteContract = read('scripts/test-backend-route-contract.mjs')
...
assertIncludes(backendGate, 'scripts/test-backend-route-contract.mjs', 'backend gate must always run the route contract')
...
assertIncludes(backendRouteContract, 'fn compat_route(', 'backend route contract must require the shared compat route helper')
assertIncludes(backendRouteContract, 'format!("/api{legacy_path}")', 'backend route contract must require derived /api paths')
assertIncludes(backendRouteContract, 'router must not hand-register /api routes outside compat_route', 'backend route contract must reject hand-registered /api routes')
assertIncludes(backendRouteContract, 'documented_routes_are_live_on_legacy_and_api_paths', 'backend route contract must require live legacy/api route parity tests')
...
assertIncludes(ciWorkflow, 'release-gate-summary:', 'GitHub workflow must include a final release candidate summary job')
assertIncludes(ciWorkflow, 'name: Release candidate summary', 'release summary job must have an explicit readable name')
assertIncludes(ciWorkflow, 'if: ${{ always() && inputs.release_candidate }}', 'release summary job must run even when required release gates fail')
assertIncludes(ciWorkflow, '- release-candidate-preflight', 'release summary job must depend on release candidate preflight')
assertIncludes(ciWorkflow, '- backend', 'release summary job must include backend gate result')
assertIncludes(ciWorkflow, '- frontend', 'release summary job must include frontend gate result')
assertIncludes(ciWorkflow, '- frontend-e2e', 'release summary job must include frontend E2E gate result')
assertIncludes(ciWorkflow, '- migration', 'release summary job must include migration gate result')
assertIncludes(ciWorkflow, '- tauri-app', 'release summary job must include Tauri gate result')
assertIncludes(ciWorkflow, '- docker-release', 'release summary job must include Docker gate result')
assertIncludes(ciWorkflow, 'GITHUB_STEP_SUMMARY', 'release summary job must write a human-readable GitHub summary')
assertIncludes(ciWorkflow, 'Release candidate gate summary', 'release summary job must write a release gate summary heading')
assertIncludes(ciWorkflow, 'FAIL: ${label} result is ${result}; release candidate is not shippable.', 'release summary job must fail on any non-success required gate')
assertIncludes(ciWorkflow, 'GitHub release candidate summary passed.', 'release summary job must print an explicit success marker')
