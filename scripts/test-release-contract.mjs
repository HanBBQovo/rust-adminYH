...
const backendAuthContract = read('scripts/test-backend-auth-contract.mjs')
const backendSqlHelperContract = read('scripts/test-backend-sql-helper-contract.mjs')
const backendPaginationContract = read('scripts/test-backend-pagination-contract.mjs')
const backendValidationContract = read('scripts/test-backend-validation-contract.mjs')
const backendResponseContract = read('scripts/test-backend-response-contract.mjs')
...
const frontendArchitectureContract = read('scripts/test-frontend-architecture.mjs')
const frontendHeaderActionContract = read('scripts/test-frontend-header-action-contract.mjs')
...
assertIncludes(backendGate, 'scripts/test-backend-validation-contract.mjs', 'backend gate must always run the validation contract')
assertIncludes(backendGate, 'scripts/test-backend-response-contract.mjs', 'backend gate must always run the response contract')
...
assertIncludes(backendAuthContract, 'pub enum AuthPolicy', 'backend auth contract must require centralized AuthPolicy')
assertIncludes(backendAuthContract, 'pub async fn require_policy', 'backend auth contract must require centralized require_policy')
assertIncludes(backendAuthContract, 'role_id\\s*==\\s*1', 'backend auth contract must reject hard-coded admin role checks')
assertIncludes(backendAuthContract, 'require_self_or_admin(&state, &headers, user_id).await?', 'backend auth contract must lock self-or-admin password policy')
assertIncludes(backendSqlHelperContract, 'repositories/sql.rs', 'backend SQL helper contract must scan the shared repository SQL helper module')
assertIncludes(backendSqlHelperContract, 'pub(super) fn db_error', 'backend SQL helper contract must require shared db_error')
assertIncludes(backendSqlHelperContract, 'pub(super) async fn fetch_count', 'backend SQL helper contract must require shared fetch_count')
assertIncludes(backendSqlHelperContract, 'get_optional_i64', 'backend SQL helper contract must require nullable i64 row decoding')
assertIncludes(backendSqlHelperContract, 'duplicate helper definitions outside repositories/sql.rs', 'backend SQL helper contract must reject copied repository helper functions')
assertIncludes(backendSqlHelperContract, 'AppError::Database(error.to_string())', 'backend SQL helper contract must reject copied database error mapping')
...
assertIncludes(backendValidationContract, 'pub fn normalize_order_mutation', 'backend validation contract must require centralized order validation')
assertIncludes(backendValidationContract, 'normalize_user_create(input)?', 'backend validation contract must require centralized user validation usage')
assertIncludes(backendValidationContract, 'normalize_role_assignment(input)?', 'backend validation contract must require centralized role assignment validation')
assertIncludes(backendValidationContract, 'JSON handlers must not own business validation', 'backend validation contract must reject handler-owned JSON validation')
assertIncludes(backendResponseContract, 'StatusJsonResponse', 'backend response contract must require status-aware response wrappers')
assertIncludes(backendResponseContract, 'LegacyDataResponse', 'backend response contract must require legacy data-only response wrappers')
assertIncludes(backendResponseContract, 'Json\\s*\\(\\s*ApiResponse::', 'backend response contract must reject handler-local ApiResponse JSON')
assertIncludes(backendResponseContract, 'impl\\s+IntoResponse\\s+for\\s+\\w+', 'backend response contract must reject handler-local response wrappers')
assertIncludes(rebuildPlan, 'scripts/test-backend-sql-helper-contract.mjs', 'rebuild plan must document the backend SQL helper contract')
assertIncludes(rebuildPlan, 'scripts/test-backend-auth-contract.mjs', 'rebuild plan must document the backend auth contract')
assertIncludes(rebuildPlan, 'crates/admin-db/src/repositories/sql.rs', 'rebuild plan must document the shared repository SQL helper module')
...
assertIncludes(rebuildPlan, 'scripts/test-backend-validation-contract.mjs', 'rebuild plan must document the backend validation contract')
assertIncludes(rebuildPlan, 'crates/admin-core/src/validation.rs', 'rebuild plan must document the centralized validation module')
assertIncludes(rebuildPlan, 'scripts/test-backend-response-contract.mjs', 'rebuild plan must document the backend response contract')
assertIncludes(rebuildPlan, 'StatusJsonResponse', 'rebuild plan must document the status-aware response wrapper')
assertIncludes(rebuildPlan, 'LegacyDataResponse', 'rebuild plan must document the legacy data-only response wrapper')
...
assertIncludes(migrationSmoke, 'Assert legacy audit report', 'migration smoke must assert inspect-old dirty legacy audit markers')
assertIncludes(migrationSmoke, 'Migration smoke legacy audit assertions OK', 'migration smoke must print an explicit dirty audit assertion success marker')
assertIncludes(migrationSmoke, 'duplicate(inspectOld, \'memory.name\')', 'migration smoke must assert duplicate memory.name audit output')
assertIncludes(migrationSmoke, 'role_permission.role_id_permission_id', 'migration smoke must assert duplicate role_permission audit output')
assertIncludes(migrationSmoke, 'orphan(inspectOld, \'company_order.order_id\')', 'migration smoke must assert orphan company_order audit output')
assertIncludes(migrationSmoke, 'orphan(inspectOld, \'receipt.oddnumber\')', 'migration smoke must assert orphan receipt oddnumber audit output')
assertIncludes(migrationSmoke, 'orphan(inspectOld, \'avatar.user_id\')', 'migration smoke must assert orphan avatar user audit output')
assertIncludes(migrationSmoke, '异常回收', 'migration smoke must assert non-standard receipt status distribution output')
...
assertIncludes(migrationFixture, 'MIG-SMOKE-ORPHAN-RECEIPT', 'migration fixture must seed an orphan receipt oddnumber audit marker')
assertIncludes(migrationFixture, 'orphan-avatar-smoke.jpg', 'migration fixture must seed an orphan avatar user audit marker')
assertIncludes(migrationFixture, '迁移收货人', 'migration fixture must seed duplicate memory audit markers')
assertIncludes(migrationFixture, '已接收', 'migration fixture must preserve legacy received receipt status')
assertIncludes(migrationFixture, '已发放', 'migration fixture must preserve legacy issued receipt status')
...
assertIncludes(frontendGate, 'scripts/test-frontend-architecture.mjs', 'frontend gate must run the architecture contract')
assertIncludes(frontendGate, 'scripts/test-frontend-header-action-contract.mjs', 'frontend gate must run the header action contract')
assertIncludes(frontendArchitectureContract, 'ResourceRegistry.tsx', 'frontend architecture contract must lock the resource registry page')
assertIncludes(frontendArchitectureContract, 'DataTableSurface', 'frontend architecture contract must require resource registry to use DataTableSurface')
assertIncludes(frontendArchitectureContract, 'toolbar={', 'frontend architecture contract must require resource registry to keep search in the DataTableSurface toolbar slot')
assertIncludes(frontendArchitectureContract, 'businessVisualTokenRules', 'frontend architecture contract must scan business visual token drift')
assertIncludes(frontendArchitectureContract, 'raw hex 颜色', 'frontend architecture contract must reject raw hex color drift in business pages')
assertIncludes(frontendArchitectureContract, 'Tailwind 任意颜色值', 'frontend architecture contract must reject arbitrary Tailwind color drift in business pages')
assertIncludes(frontendArchitectureContract, 'Tailwind 任意阴影值', 'frontend architecture contract must reject arbitrary Tailwind shadow drift in business pages')
assertIncludes(rebuildPlan, 'ResourceRegistry', 'rebuild plan must document the ResourceRegistry table shell rule')
assertIncludes(rebuildPlan, 'DataTableSurface', 'rebuild plan must document the shared table surface rule')
assertIncludes(rebuildPlan, '业务页面和业务组件不得散写 raw hex', 'rebuild plan must document the business visual token rule')
assertIncludes(frontendHeaderActionContract, 'HeaderActionButton', 'frontend header action contract must require the shared header action button')
assertIncludes(frontendHeaderActionContract, 'PageShell actions must not hand-roll Button controls', 'frontend header action contract must reject page header Button drift')
assertIncludes(frontendHeaderActionContract, '导出筛选结果', 'frontend header action contract must preserve order export header action')
assertIncludes(frontendHeaderActionContract, 'refreshRoles()', 'frontend header action contract must preserve user role refresh behavior')
assertIncludes(rebuildPlan, 'scripts/test-frontend-header-action-contract.mjs', 'rebuild plan must document the frontend header action contract')
assertIncludes(rebuildPlan, 'HeaderActionButton', 'rebuild plan must document the shared page header action wrapper')
...
assertOccurrences(ciWorkflow, 'install-frontend-deps.sh', 3, 'GitHub release workflow direct npm installs must delegate to the shared frontend dependency installer; frontend gates use scripts/test-frontend.sh')
...
assertIncludes(frontendGate, 'GITHUB_FRONTEND_E2E_SEPARATE_JOB', 'frontend gate must support the dedicated GitHub E2E job without silently weakening local release gates')
