...
assertIncludes(dockerGate, 'Docker base image preflight', 'Docker gate must pre-pull base images before reporting build failures')
assertIncludes(dockerGate, 'docker image inspect "$image"', 'Docker gate must reuse cached base images before hitting remote registries')
assertIncludes(dockerGate, 'docker pull "$image"', 'Docker gate must make registry pull failures explicit before Dockerfile builds')
assertIncludes(dockerGate, 'DOCKER_REGISTRY_PREFIX=docker.1ms.run/library scripts/test-docker.sh', 'Docker gate must document a registry-prefix fallback for Docker Hub auth/network failures')
assertIncludes(dockerGate, 'exit 126', 'Docker gate must use a distinct exit code for base image pull failures')
assertIncludes(dockerGate, 'Docker host port preflight', 'Docker gate must check host ports before long image builds')
assertIncludes(dockerGate, 'port_is_listening', 'Docker gate must detect host port conflicts before compose startup')
assertIncludes(dockerGate, 'API_PORT=16825 WEB_PORT=18081 MYSQL_PORT=33307 scripts/test-docker.sh', 'Docker gate must document alternate host port overrides')
assertIncludes(dockerGate, 'exit 124', 'Docker gate must use a distinct exit code for host port conflicts')
