...
当前仓库已提供 `scripts/check-all.sh`、`scripts/test-backend.sh`、`scripts/test-frontend.sh`、`scripts/test-migration.sh` 作为统一质量门禁入口。默认情况下，Rust 脚本使用 `CARGO_OFFLINE=true`，保证在依赖已缓存但网络不稳定时仍可稳定运行；需要在线拉取新依赖时可显式执行 `CARGO_OFFLINE=false scripts/check-all.sh`。GitHub `frontend` job 必须直接运行 `scripts/test-frontend.sh`，不能手写 `npm run lint/typecheck/test/build` 绕过 architecture、header action、pagination、mutation、detail loader 和 async action 静态合同；Playwright E2E 由独立 `frontend-e2e` job 安装浏览器后执行，`frontend` job 只允许通过 `GITHUB_FRONTEND_E2E_SEPARATE_JOB=true` 表明 E2E 已由专门 job 覆盖。`scripts/test-release-contract.mjs` 已接入默认门禁，用静态契约锁定 `RELEASE_GATE=true` 必须开启的真实 MySQL、迁移、可重建迁移 smoke、头像文件、E2E、coverage、Docker、Tauri 和 sidecar smoke 要求，防止后续把发布候选改成“跳过也通过”；`scripts/test-release-preflight.mjs` 会实际执行多组 `RELEASE_GATE=true scripts/check-all.sh` 负向用例，确认缺少关键发布变量时会在 Backend 前快速失败，而不是只靠字符串扫描。`scripts/test-backend-mysql-contract.mjs` 也已接入后端默认门禁，会扫描所有 `crates/admin-db/tests/mysql_*.rs` 和 `crates/admin-api/tests/mysql_*.rs`，确认每个真实 MySQL 回归都在 `RUN_DB_TESTS=true` 分支由 `run_mysql_tests` 自动发现并以 `-- --ignored` 执行，并保持默认 ignored，防止新增仓储/API 真实库测试却没有进入发布候选门禁。`scripts/test-migration.sh` 默认会执行 `admin-migration rollback-plan --format json`，确保没有真实数据库连接时也能持续校验回滚文档出口；`RUN_MIGRATION_SMOKE=true scripts/test-migration.sh` 会进一步调用 `scripts/test-migration-smoke.sh`，用 Docker MySQL 重建 `admin_yh_smoke_old` / `admin_yh_smoke_new`，导入 `scripts/seed-migration-fixture.sql`，并保存 rollback-plan、inspect-old、dry-run、apply、verify 和 verify-files JSON 报告。迁移 smoke fixture 同时保留旧库常见脏数据审计 marker，包括重复 `memory.name`、重复 `role_permission(role_id, permission_id)`、孤儿 `company_order.order_id`、孤儿 `receipt.oddnumber`、孤儿 `avatar.user_id` 和非标准回单状态；脚本会解析 `inspect-old` JSON 并断言这些重复、孤儿和状态分布被报告，防止可重建 smoke 只覆盖干净 happy path。设置 `RELEASE_ARTIFACT_DIR` 时，迁移 smoke、Docker gate 和 Tauri gate 会把 JSON/health/manifest/hash/产物清单统一写到该目录；GitHub 手动 workflow 会上传 `release-gate-migration`、`release-gate-docker`、`release-gate-tauri` artifacts，作为发布候选留档。头像文件迁移校验已经从“只报告差异”升级为阻断门禁：`verify-files` 会输出 `status=passed/failed`，缺失、多余或 SHA256 变化都会让 CLI 非 0 退出，发布前必须修复后复验。
