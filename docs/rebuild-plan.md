...
- Receipt：`POST /receipt/list`、`PATCH /receipt/:receiptId`、`PATCH /receipt/batch/status`、`POST /notrecovery/list`、`POST /recovery/list`
...
- database 层统一管理 SQLx pool、事务、分页参数、迁移；`admin-db` 生产仓储不得直接散写 `pool.begin()`、`tx.commit()` 或 `sqlx::Transaction<'_, MySql>`，必须通过 `admin-db::transaction::{begin_mysql_transaction, commit_mysql_transaction, with_mysql_transaction, MySqlTransaction}` 统一封装事务入口、提交、rollback-on-drop 诊断和错误上下文。`MySqlTransaction` 必须保存稳定事务 scope，`commit_mysql_transaction(tx)` 只能消费带 scope 的事务对象，仓储层不允许在 commit 时重复传 scope 字符串。多步骤事务块优先使用 `with_mysql_transaction(pool, scope, |tx| ...)` 统一 begin/body/commit；事务体内关键 SQL 必须用 `transaction_sql_error(tx.scope(), operation, error)` 标出业务操作，例如 `fetch_existing_receipt_ids`、`delete_role_permissions`。事务 scope 必须使用稳定的业务名，例如 `order.create`、`user.update_avatar`、`role.replace_menu_ids`，便于日志和测试定位。列表分页必须通过 `admin-db::pagination::{Page, push_limit_offset}` 统一构造和绑定 `LIMIT/OFFSET`，禁止 repository 直接拼接或绑定分页 SQL；默认门禁必须执行 `scripts/test-backend-pagination-contract.mjs` 静态阻断回归。
- repository SQL 行读取、列表 count 解码和数据库错误映射必须集中在 `crates/admin-db/src/repositories/sql.rs`，业务仓储只能通过 `super::sql::{db_error, fetch_count, get_nullable_string, get_string, get_i32, get_i64, get_optional_i64}` 使用共享 helper；禁止各 repository 重新定义 `db_error`、`fetch_count`、`get_string`、`get_i64`、`get_i32`、`get_nullable_string`、`get_optional_i64` 或复制 `AppError::Database(error.to_string())` / nullable row decoding 逻辑。默认后端门禁必须执行 `scripts/test-backend-sql-helper-contract.mjs`，静态锁住共享 SQL helper 边界，保证后续维护只改一处。
- auth/permission 独立封装，业务模块只能调用统一权限接口，不能硬编码 `role_id == 1`；HTTP 层权限判断必须通过 `AuthPolicy` / `require_policy` 收口，`require_admin` 和 `require_self_or_admin` 只能作为语义化薄封装。默认门禁必须执行 `scripts/test-backend-auth-contract.mjs`，静态阻断 handler 直接调用 `is_super_admin`、导入 `SUPER_ADMIN_ROLE_ID`、硬编码 `role_ids.contains(&1)`，并锁住写接口继续通过统一 auth policy wrapper。
- error/response 独立封装，所有 API 返回统一结构，不允许每个 handler 自己拼错误格式或直接 `Json(ApiResponse::...)`；默认成功响应走 `JsonResponse`，中文成功文案走 `MessageResponse`，健康检查这种需要保留业务 envelope 但改变 HTTP 状态的接口走 `StatusJsonResponse`，旧 `memory` data-only 兼容接口走 `LegacyDataResponse`。默认门禁必须执行 `scripts/test-backend-response-contract.mjs`，静态阻断 handler-local `IntoResponse` wrapper、手写 `StatusCode::OK + Json(...)` 和绕过 `response.rs` 的 envelope 拼装。
- route/compat 独立封装，旧路径和 `/api` 新兼容路径必须通过 `compat_route` 一次声明、自动双注册；业务路由不得散落手写 `.route("/api/...")`，避免新增接口时只注册 legacy 或只注册 `/api` 一侧。默认门禁必须执行 `scripts/test-backend-route-contract.mjs`，静态阻断手写 `/api` 路由、传入已带 `/api` 前缀的 legacy path，并要求 `route_compatibility.rs` 继续用 live request 验证 legacy 与 `/api` 两侧都已注册且响应同形。
- validation 独立封装，新增/编辑订单、用户、角色、回单状态必须走 `crates/admin-core/src/validation.rs` 统一 normalization/校验入口，service 只负责调用命名校验函数并继续执行业务副作用，handler 不允许拥有普通 JSON 业务校验；头像上传这类 multipart 传输层文件检查可以留在 handler，但必须只处理字段名、文件大小、MIME/扩展名和落盘前字节读取。默认后端门禁必须执行 `scripts/test-backend-validation-contract.mjs`，静态锁住订单、用户、角色、回单状态校验不再散回 service 私有 helper 或 JSON handler。
...
- `auth`：登录、退出、token 校验；验证码功能不进入新系统。
...
- 页面保存、删除、批量状态流转、订单筛选导出、账户改密、头像上传和设置偏好保存/恢复等异步业务动作必须逐步收敛到 `src/lib/use-mutation-action.ts` 或更具体的业务 hook，由封装统一处理 pending、success/error toast、确认弹窗、成功后刷新和 settled 清理回调；业务页面只保留实体参数、旧接口文案和特殊业务规则。当前 `AccountPreferences、CompaniesList、OrdersList、ReceiptsList、RolesList、MenusList、Settings、UsersList` 已作为样板迁入该 hook；其中 OrdersList 的新增/编辑/删除和筛选导出均由共享 mutation hook 管理 pending 与 toast，AccountPreferences 的头像上传成功回调和文件输入清理也通过共享 mutation hook 收敛。回单页仍保留 `updatingId` / `batchUpdating` 作为按钮粒度禁用状态，账户偏好仍保留头像类型/大小的前端本地校验，Settings 仍保留首屏偏好加载失败的页面级 message，但保存/恢复的成功/失败 toast 和刷新回调已由共享 mutation hook 收敛，避免每页/组件重复 `try/catch/finally + confirm + toast + refresh`。
- 查看/编辑弹窗的详情补拉必须通过 `src/lib/use-detail-dialog.ts` 统一处理打开新建、列表行 optimistic seed、详情请求、接口详情替换、关闭 reset 和过期响应丢弃；该 hook 内部复用 `src/lib/use-detail-loader.ts` 统一错误 toast、空详情兜底和请求序号竞态保护。当前发货公司、订单、角色、菜单、用户等列表页均通过该 hook 管理详情弹窗生命周期，避免页面内重复 `dialogOpen/dialogMode/selectedX + loadDetail/resetDetail` 样板；菜单详情通过 `mapLoaded` 保留旧树结构 normalize，用户详情通过 `seedDetail` 保留列表行角色兜底。`scripts/test-frontend-detail-loader-contract.mjs` 必须锁住这些列表页不回退到页面级 `try/catch + showToast` 详情加载样板。
- 登录等需要表单内错误文案、不能弹全局 toast 的本地异步流程必须通过 `src/lib/use-async-action.ts` 统一处理 pending/error/success callback；Login 只保留账号、密码、记住账号和 `onAuthenticated` 业务语义，不再展示或请求验证码。`scripts/test-frontend-async-action-contract.mjs` 必须锁住 Login 不回退到页面级 `setSubmitting/setError + try/catch/finally`，并持续验证只保存账号名、不保存任何 password key。
...
当前仓库已提供 `scripts/check-all.sh`、`scripts/test-backend.sh`、`scripts/test-frontend.sh`、`scripts/test-migration.sh` 作为统一质量门禁入口。默认情况下，Rust 脚本使用 `CARGO_OFFLINE=true`，保证在依赖已缓存但网络不稳定时仍可稳定运行；需要在线拉取新依赖时可显式执行 `CARGO_OFFLINE=false scripts/check-all.sh`。前端已经从 `frontend-template/web` 派生到 `apps/desktop/web`，所以 `scripts/test-frontend.sh` 默认不再允许“前端目录不存在或缺少 package.json”时静默 `SKIP`；如果 `WEB_DIR` 指错、前端被误删或模板迁入回退，默认门禁必须失败。只有早期初始化或临时脚手架验证场景可以显式设置 `ALLOW_MISSING_FRONTEND=true` 跳过，且该逃生开关在 `RELEASE_GATE=true` 下无效。GitHub `frontend` job 必须直接运行 `scripts/test-frontend.sh`，不能手写 `npm run lint/typecheck/test/build` 绕过 architecture、header action、pagination、mutation、detail loader 和 async action 静态合同；Playwright E2E 由独立 `frontend-e2e` job 安装浏览器后执行，`frontend` job 只允许通过 `GITHUB_FRONTEND_E2E_SEPARATE_JOB=true` 表明 E2E 已由专门 job 覆盖。`scripts/test-release-contract.mjs` 已接入默认门禁，用静态契约锁定 `RELEASE_GATE=true` 必须开启的真实 MySQL、迁移、可重建迁移 smoke、头像文件、E2E、coverage、Docker、Tauri 和 sidecar smoke 要求，同时锁住前端缺失不能默认跳过，防止后续把发布候选或日常门禁改成“跳过也通过”；`scripts/test-release-preflight.mjs` 会实际执行多组 `RELEASE_GATE=true scripts/check-all.sh` 和子门禁负向用例，确认缺少关键发布变量或缺少已迁入前端时会快速失败，而不是只靠字符串扫描。`scripts/test-backend-mysql-contract.mjs` 也已接入后端默认门禁，会扫描所有 `crates/admin-db/tests/mysql_*.rs` 和 `crates/admin-api/tests/mysql_*.rs`，确认每个真实 MySQL 回归都在 `RUN_DB_TESTS=true` 分支由 `run_mysql_tests` 自动发现并以 `-- --ignored` 执行，并保持默认 ignored，防止新增仓储/API 真实库测试却没有进入发布候选门禁。`scripts/test-migration.sh` 默认会执行 `admin-migration rollback-plan --format json`，确保没有真实数据库连接时也能持续校验回滚文档出口；`RUN_MIGRATION_SMOKE=true scripts/test-migration.sh` 会进一步调用 `scripts/test-migration-smoke.sh`，用 Docker MySQL 重建 `admin_yh_smoke_old` / `admin_yh_smoke_new`，导入 `scripts/seed-migration-fixture.sql`，并保存 rollback-plan、inspect-old、dry-run、apply、verify 和 verify-files JSON 报告。迁移 smoke fixture 同时保留旧库常见脏数据审计 marker，包括重复 `memory.name`、重复 `role_permission(role_id, permission_id)`、孤儿 `company_order.order_id`、孤儿 `receipt.oddnumber`、孤儿 `avatar.user_id` 和非标准回单状态；脚本会解析 `inspect-old` JSON 并断言这些重复、孤儿和状态分布被报告，防止可重建 smoke 只覆盖干净 happy path。设置 `RELEASE_ARTIFACT_DIR` 时，迁移 smoke、Docker gate 和 Tauri gate 会把 JSON/health/manifest/hash/产物清单统一写到该目录；GitHub 手动 workflow 会上传 `release-gate-migration`、`release-gate-docker`、`release-gate-tauri` artifacts，作为发布候选留档。头像文件迁移校验已经从“只报告差异”升级为阻断门禁：`verify-files` 会输出 `status=passed/failed`，缺失、多余或 SHA256 变化都会让 CLI 非 0 退出，发布前必须修复后复验。

发布候选必须显式使用 `RELEASE_GATE=true scripts/check-all.sh`。该模式会在执行前阻断任何“看起来通过但实际跳过发布级验证”的组合：Rust workspace `Cargo.toml`、`apps/desktop/web/package.json` 和 Tauri workspace `apps/desktop/src-tauri/Cargo.toml` 都必须存在，缺任一项都要在 release preflight 阶段失败，不能沿用日常初始化阶段的 `SKIP`；同时必须设置 `RUN_DB_TESTS=true`、`RUN_DB_TESTS_ISOLATED=true`、`ADMIN_DB_TEST_DATABASE_URL`、`OLD_DATABASE_URL`、`NEW_DATABASE_URL`、`MIGRATION_APPLY=true`、`RUN_MIGRATION_SMOKE=true`、`NEW_AVATAR_DIR`、`RUN_E2E=true`、`RUN_COVERAGE=true`、`RUN_DOCKER=true`、`RUN_DOCKER_E2E=true`、`RUN_TAURI=true`、`RUN_TAURI_DMG=true`、`RUN_TAURI_SIDECAR_SMOKE=true`，并提供 `TAURI_SIDECAR_DATABASE_URL` 或 `DATABASE_URL` 指向可重建测试库。这样本地日常提交仍可运行轻量门禁，但发布前真实 MySQL repository、后端可重建隔离 MySQL smoke、迁移 dry-run、可重建迁移 smoke、从旧库 apply 到 fresh 新库、迁移 verify、头像文件校验、mock/本地 Playwright、覆盖率、Docker compose 健康检查、Docker Web + Rust API + MySQL 真实浏览器 E2E、Tauri `.app`/DMG 打包和打包后 sidecar 真实 `/api/health` smoke 都不能被默认跳过。直接执行子门禁也必须遵守同一 release 语义：`RELEASE_GATE=true scripts/test-backend.sh` 不能跳过 `RUN_DB_TESTS_ISOLATED=true`，`RELEASE_GATE=true scripts/test-frontend.sh` 不能跳过 coverage/E2E，`RELEASE_GATE=true scripts/test-docker.sh` 不能跳过 Docker 真实浏览器 E2E，`RELEASE_GATE=true scripts/test-tauri-build.sh` 不能跳过 DMG 或打包后 sidecar smoke；单独开启 `RUN_DOCKER_E2E=true` 必须同时开启 `RUN_DOCKER=true`，单独开启 `RUN_TAURI_DMG=true` 或 `RUN_TAURI_SIDECAR_SMOKE=true` 必须同时开启 `RUN_TAURI=true`。若本机 GUI/Finder/hdiutil 环境导致 DMG 失败，必须单独记录失败原因、命令和 `.app` 替代验证结果，不允许把 `RUN_TAURI_DMG=false` 当作发布级通过。

前端质量门禁必须包含 `lint`、架构扫描、分页封装契约、`typecheck`、`test`、`build` 六段；`scripts/test-frontend-architecture.mjs` 会阻断页面/业务组件直接 `fetch`、引入 `axios`、绕过 `src/api/*` 调用 `apiRequest`、在业务层直接引入 Radix 原语、在业务页面散写 inline style、业务代码直接读写 Web Storage、生产页面遗留“真实项目/模板演示/参考页”等假实现文案，并静态锁住 `ResourceRegistry` 必须复用 `DataTableSurface` 和 `toolbar` 插槽，确保请求封装、本地偏好封装和 `frontend-template` 组件风格不会在后续页面中退化。`scripts/test-frontend-pagination-contract.mjs` 会单独锁住分页列表封装：分页列表页必须导入并调用 `usePaginatedResource`，由 hook 返回 `rows/total/page/pageSize/pagination`，并把 `pagination={pagination}` 传给 `DataTableSurface`；脚本会禁止页面重新手写 `page` state、绕过 hook 用 `useResource(() => listUsers/listRoles/listCompanies/listOrders/listReceipts(...))` 拉分页列表、手写 `{ page, pageSize, total, onPageChange }`，同时放行菜单树、资源注册表、工作台统计、角色授权弹窗、用户角色下拉和订单导出这类非分页或非主列表场景。Dashboard 最近页面记忆也必须走 `src/session/session-store.ts` 的 `readStoredPage/saveStoredPage`，不得在页面层直接读写 `localStorage`。Playwright E2E 通过 `RUN_E2E=true scripts/test-frontend.sh` 显式开启，覆盖率通过 `RUN_COVERAGE=true scripts/test-frontend.sh` 显式开启，避免本地缺少浏览器二进制或覆盖率依赖时阻塞普通提交；发布候选版本必须同时开启 E2E 和 coverage。
...
Docker/容器门禁已作为发布级测试的一部分补齐：`Dockerfile.admin-api` 构建 Rust Axum API 镜像，`Dockerfile.desktop-web` 构建模板前端静态镜像，`docker-compose.ci.yml` 组合 MySQL、API 和 Web，并通过 `DATABASE_MIGRATE_ON_START=true` 在 CI 数据库启动时应用兼容 schema，用于验证镜像能启动并通过 `/api/health` 与首页检查。默认 `scripts/check-all.sh` 会先执行轻量 `scripts/test-docker-contract.mjs`，静态锁定 Dockerfile、compose、nginx `/api` 代理、非 root 运行、迁移开关、健康检查和失败诊断契约；重型镜像构建和 compose smoke 仍需显式设置 `RUN_DOCKER=true`。发布候选版本必须执行 `RUN_DOCKER=true RUN_DOCKER_E2E=true scripts/check-all.sh`。如果 Docker 构建或 compose 启动失败，`scripts/test-docker.sh` 必须输出 `docker version`、`docker compose version`、commit、镜像 tag、脱敏后的数据库连接、compose ps 和三类服务最近 200 行日志，不能只给出笼统的 “Docker build failed”。设置 `RELEASE_ARTIFACT_DIR` 时，脚本会保存 `api-health.json`、`web-health.html`、`web-api-health.json`、`image-inspect.json`、`compose-ps.json` 和 `manifest.txt`，用于发布后追溯镜像、compose、健康检查和 nginx 反代证据。

GitHub Actions 开发期不自动执行：`.github/workflows/ci.yml` 只保留 `workflow_dispatch`，push/PR 不会启动 runner。最终需要 GitHub Docker 打包时，从 Actions 页面手动运行 `CI` workflow，并勾选 `run_docker=true`；若要证明生产浏览器链路，同时勾选 `run_docker_e2e=true`。backend release job 运行在 Ubuntu runner 上，并在 `release_candidate=true` 时设置 `RUN_DB_TESTS=true RUN_DB_TESTS_ISOLATED=true ADMIN_DB_TEST_DATABASE_URL=mysql://admin_yh:admin_yh@127.0.0.1:33319/admin_yh_backend_smoke`，由 `scripts/test-backend-mysql-smoke.sh` 自建本机 compose MySQL 测试库，不再依赖外部 `ADMIN_DB_TEST_DATABASE_URL` secret。Docker job 复用 `scripts/test-docker.sh`，失败时仍输出 Docker 版本、commit、镜像 tag、脱敏数据库连接、compose 状态和服务日志，便于定位是镜像拉取、构建、启动、健康检查还是真实 E2E 失败；成功或失败都会尝试上传 `release-gate-docker` artifact。迁移 job 会上传 `release-gate-migration`，Tauri job 会上传 `release-gate-tauri`，这些报告 artifact 必须和 `.app`、`.dmg` 一起作为发布记录保存。本地前端门禁和 GitHub workflow 的所有前端依赖安装必须统一调用 `scripts/install-frontend-deps.sh`，并和 `Dockerfile.desktop-web` 使用同一套 npm 稳定参数：固定 `NPM_REGISTRY=https://registry.npmjs.org`、`--replace-registry-host=always`、`--fetch-retries=5`、`--fetch-retry-mintimeout=20000`、`--fetch-retry-maxtimeout=120000`，避免 lockfile 中 `registry.npmjs.org` 与 `registry.npmmirror.com` 混用导致新机器、runner 或清理依赖后又卡在依赖安装阶段。最终需要 GitHub Tauri 打包时，手动勾选 `run_tauri=true`；需要 macOS 安装包时同时勾选 `run_tauri_dmg=true`；需要发布级打包后 sidecar smoke 时同时勾选 `run_tauri_sidecar_smoke=true`，并提前配置仓库 secret `TAURI_SIDECAR_DATABASE_URL` 或 `DATABASE_URL` 指向可重建 MySQL 测试库。`run_tauri_dmg=true` 或 `run_tauri_sidecar_smoke=true` 但未同时开启 `run_tauri=true` 会在 preflight 阶段失败；只要开启 sidecar smoke，即使不是 `release_candidate=true`，也会先检查 sidecar 测试库 secret，避免 macOS 构建完成后才暴露配置缺失。开启 sidecar smoke 但未配置 secret 时失败是正确发布阻断，不允许静默降级。

GitHub 发布候选必须显式勾选 `release_candidate=true`，并同时勾选 `run_docker=true`、`run_docker_e2e=true`、`run_tauri=true`、`run_tauri_dmg=true`、`run_tauri_sidecar_smoke=true`。`release-candidate-preflight` job 会在任何重型 job 前先检查这些开关，以及 `OLD_DATABASE_URL`、`NEW_DATABASE_URL`、`NEW_AVATAR_DIR` secret/variable、`TAURI_SIDECAR_DATABASE_URL` 或 `DATABASE_URL` secret 是否存在，但不会打印 secret 值；后端 MySQL 回归由 runner 本机 compose smoke 生成临时库，不需要 `ADMIN_DB_TEST_DATABASE_URL` secret。该模式会把 backend job 提升为 `RELEASE_GATE=true RUN_DB_TESTS=true RUN_DB_TESTS_ISOLATED=true`，让 frontend job 额外跑 `npm run test:coverage`，并把 migration job 接到真实旧库/新库和头像目录校验，同时设置 `MIGRATION_APPLY=true RUN_MIGRATION_SMOKE=true`，证明当前迁移程序能从旧库真实写入 fresh 新库并在 apply 后复验，也能用内置 fixture 在 GitHub runner 上重建迁移 smoke；`release-gate-summary` 会在发布候选模式下用 `always()` 汇总 preflight、backend、frontend、frontend-e2e、migration、tauri-app 和 docker-release 的结果，写入 GitHub Step Summary，并在任一必需 job 不是 `success` 时失败，避免多个分散 job 让发布结论不清晰；如果 GitHub billing/spending limit 让 runner 无法启动，失败不代表 Docker 或 Tauri 构建错误，必须先修复账号 Billing 后再重新手动运行。
...
Docker Web 运行态门禁不只检查首页 HTML：`desktop-web` compose 服务必须带 nginx healthcheck，`scripts/test-docker.sh` 必须同时检查 `http://127.0.0.1:18080/`、首页引用的至少一个生产静态资源，以及经 nginx 代理后的 `http://127.0.0.1:18080/api/health`。这样 Docker 发布候选能覆盖 Web 静态资源、SPA fallback 和 `/api` 反代链路，而不是只证明容器进程存在。如果本机端口已被其他项目占用，使用 `WEB_PORT=<port>`、`API_PORT=<port>` 或 `MYSQL_PORT=<port>` 覆盖宿主端口；`API_URL`、`WEB_URL` 和 `WEB_API_URL` 默认会跟随对应端口派生，只有需要访问非本机地址时才额外覆盖 URL。

Docker 基础镜像允许通过环境变量覆盖：`RUST_IMAGE`、`RUNTIME_IMAGE`、`NODE_IMAGE`、`NGINX_IMAGE`、`MYSQL_IMAGE`。本机或 CI 如果 Docker Hub 直连不稳定，可以设置 `DOCKER_REGISTRY_PREFIX=<registry>/library` 统一改写 Docker Hub library 镜像，例如 `DOCKER_REGISTRY_PREFIX=docker.m.daocloud.io/library scripts/test-docker.sh`；也可以继续用单个镜像变量指向内部镜像。脚本会在诊断中打印实际使用的基础镜像、registry prefix、宿主端口和镜像 tag，便于定位是 Docker daemon 未启动、网络拉取失败、构建失败、启动失败还是健康检查失败。
...
| 迁移变更 | `RUN_MIGRATION_SMOKE=true OLD_DATABASE_URL=mysql://... NEW_DATABASE_URL=mysql://... NEW_AVATAR_DIR=/tmp/admin_yh_avatar_shadow scripts/test-migration.sh` | 可重建迁移 smoke、迁移 dry-run、目标库 `targetPreflight` 空库/表结构预检、rollback-plan、头像文件 verify 和影子库对账可重复执行 | 未设置 `MIGRATION_APPLY=true` 时不执行真实外部影子库 apply；smoke 只证明内置 fixture 的迁移闭环 |
| Docker 发布候选 | `RUN_DOCKER=true scripts/check-all.sh` | API/Web 镜像构建、compose MySQL/API/Web 健康检查通过 | 不代表桌面 `.app` 或 DMG 可安装 |
| GitHub 发布候选 | Actions 手动运行 `CI`，勾选 `release_candidate=true`、`run_docker=true`、`run_docker_e2e=true`、`run_tauri=true`、`run_tauri_dmg=true`、`run_tauri_sidecar_smoke=true`，并配置真实 MySQL/迁移/头像/sidecar secrets | 先由 `release-candidate-preflight` 阻断缺失开关或 secret，再在 GitHub runner 上执行真实 DB、coverage、可重建迁移 smoke、迁移 apply+verify、Docker E2E、Tauri `.app`/DMG 和 bundled sidecar smoke | 需要 GitHub billing/spending limit 正常；preflight 只检查 secret 存在，不打印 secret，也不替代真实业务验收 |
...
发布候选版本至少按顺序执行：默认门禁 -> Playwright E2E -> coverage -> 真实 MySQL repository -> 可重建迁移 smoke -> 迁移 dry-run/apply/verify -> Docker -> Tauri `.app` -> DMG。任何一步失败都必须记录 commit、命令、脱敏环境变量、失败阶段、关键日志和是否允许替代验证。
...
2. 准备可重建的 MySQL 测试库和头像影子目录，执行 `RUN_DB_TESTS=true ADMIN_DB_TEST_DATABASE_URL=... scripts/check-all.sh`，再执行 `RUN_MIGRATION_SMOKE=true OLD_DATABASE_URL=... NEW_DATABASE_URL=... NEW_AVATAR_DIR=... MIGRATION_APPLY=true scripts/test-migration.sh`，证明真实仓储、可重建迁移 smoke、迁移 apply、verify 和头像文件校验闭环通过。
...
6. 在本地组合执行一次完整 `RELEASE_GATE=true scripts/check-all.sh`，证明发布候选不会跳过真实 MySQL、可重建迁移 smoke、迁移 apply/verify、头像校验、E2E、coverage、Docker、Tauri、DMG 和 bundled sidecar smoke；缺少任一环境变量或开关时 preflight 失败是正确阻断。
