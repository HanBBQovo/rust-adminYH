...
- 页面保存、删除、批量状态流转等异步业务动作必须逐步收敛到 `src/lib/use-mutation-action.ts` 或更具体的业务 hook，由封装统一处理 pending、success/error toast、确认弹窗和成功后刷新；业务页面只保留实体参数、旧接口文案和特殊业务规则。当前 `CompaniesList、RolesList、MenusList` 已作为样板迁入该 hook，后续订单、用户和回单动作必须按单功能提交继续迁移，避免每页重复 `try/catch/finally + confirm + toast + refresh`。
