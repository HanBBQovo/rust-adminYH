...
GitHub 发布候选必须显式勾选 `release_candidate=true`，并同时勾选 `run_docker=true`、`run_docker_e2e=true`、`run_tauri=true`、`run_tauri_dmg=true`、`run_tauri_sidecar_smoke=true`。`release-candidate-preflight` job 会在任何重型 job 前先检查这些开关，以及 `ADMIN_DB_TEST_DATABASE_URL`、`OLD_DATABASE_URL`、`NEW_DATABASE_URL`、`NEW_AVATAR_DIR` secret/variable、`TAURI_SIDECAR_DATABASE_URL` 或 `DATABASE_URL` secret 是否存在，但不会打印 secret 值。该模式会把 backend job 提升为 `RELEASE_GATE=true RUN_DB_TESTS=true`，让 frontend job 额外跑 `npm run test:coverage`，并把 migration job 接到真实旧库/新库和头像目录校验，同时设置 `MIGRATION_APPLY=true RUN_MIGRATION_SMOKE=true`，证明当前迁移程序能从旧库真实写入 fresh 新库并在 apply 后复验，也能用内置 fixture 在 GitHub runner 上重建迁移 smoke；如果 GitHub billing/spending limit 让 runner 无法启动，失败不代表 Docker 或 Tauri 构建错误，必须先修复账号 Billing 后再重新手动运行。
