...
GitHub 发布候选必须显式勾选 `release_candidate=true`，并同时勾选 `run_docker=true`、`run_docker_e2e=true`、`run_tauri=true`、`run_tauri_dmg=true`、`run_tauri_sidecar_smoke=true`。`release-candidate-preflight` job 会在任何重型 job 前先检查这些开关，以及 `ADMIN_DB_TEST_DATABASE_URL`、`OLD_DATABASE_URL`、`NEW_DATABASE_URL`、`NEW_AVATAR_DIR` secret/variable、`TAURI_SIDECAR_DATABASE_URL` 或 `DATABASE_URL` secret 是否存在，但不会打印 secret 值。该模式会把 backend job 提升为 `RELEASE_GATE=true RUN_DB_TESTS=true`，让 frontend job 额外跑 `npm run test:coverage`，并把 migration job 接到真实旧库/新库和头像目录校验；如果 GitHub billing/spending limit 让 runner 无法启动，失败不代表 Docker 或 Tauri 构建错误，必须先修复账号 Billing 后再重新手动运行。
...
| GitHub 发布候选 | Actions 手动运行 `CI`，勾选 `release_candidate=true`、`run_docker=true`、`run_docker_e2e=true`、`run_tauri=true`、`run_tauri_dmg=true`、`run_tauri_sidecar_smoke=true`，并配置真实 MySQL/迁移/头像/sidecar secrets | 先由 `release-candidate-preflight` 阻断缺失开关或 secret，再在 GitHub runner 上执行真实 DB、coverage、Docker E2E、Tauri `.app`/DMG 和 bundled sidecar smoke | 需要 GitHub billing/spending limit 正常；preflight 只检查 secret 存在，不打印 secret，也不替代真实业务验收 |
