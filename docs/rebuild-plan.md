...
- 登录等需要表单内错误文案、不能弹全局 toast 的本地异步流程必须通过 `src/lib/use-async-action.ts` 统一处理 pending/error/success callback；Login 只保留账号、密码、验证码、记住账号和 `onAuthenticated` 业务语义，验证码刷新仍保持非阻塞独立 loader。`scripts/test-frontend-async-action-contract.mjs` 必须锁住 Login 不回退到页面级 `setSubmitting/setError + try/catch/finally`，并持续验证只保存账号名、不保存任何 password key。
